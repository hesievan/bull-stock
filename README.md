...
> 每日更新的量化工具，从 **4 个维度、8 个核心指标 + CFFEX 股指期权恐慌指数** 综合评估 A 股市场整体热度水平，
...
<p align="center">
  <img src="https://img.shields.io/badge/version-v3.22-blue" alt="version">
...
## 回测验证（V2 引擎，2814 交易日全历史回测）

> 回测区间: 2015-01-05 ~ 2026-08-07 | 2814 个交易日 | 8 指标加权体系（2026-08 调权后）

### 关键牛熊转折点

| 日期 | 事件 | V2 热度 | 级别 | 上证 | 信号评价 |
|------|------|---------|------|------|---------|
| 2015-06-12 | 5178 大顶 | **91.3** | 🔴 | 5166 | 正确顶信号，极强 |
| 2015-08-26 | 股灾底 2850 | 39.3 | 🟢 | 2927 | 正确底信号 |
| 2016-01-28 | 熔断底 2638 | **15.1** | 🟢 | 2656 | 极强底信号 |
| 2018-01-29 | 蓝筹牛顶 3587 | 44.5 | 🟡 | 3523 | ⚠️ 缩量型顶部，未发出足够预警 |
| 2018-10-19 | 贸易战底 2449 | **8.8** | 🟢 | 2550 | 极强底信号 |
| 2019-04-19 | 春季顶 3288 | 58.9 | 🟠 | 3271 | 正确顶信号 |
| 2020-03-23 | 疫情底 2646 | 41.7 | 🟡 | 2660 | 信号偏中性 |
| 2021-02-18 | 核心资产顶 3731 | 64.8 | 🟠 | 3675 | 正确顶信号 |
| 2021-12-13 | 结构牛顶 | **71.4** | 🔴 | 3681 | 正确顶信号 |
| 2024-02-05 | 底部 2635 | 27.5 | 🟢 | 2702 | 正确底信号 |
| 2024-09-24 | 924 行情起点 | 30.1 | 🟢 | 2863 | 正确底信号 |
| 2024-10-08 | 924 行情顶 3674 | 59.5 | 🟠 | 3490 | 正确顶信号（调权后从黄区升至橙区） |
...
| 熊市平均热度 | 40.5 | 975 个熊市区间交易日 |
| 牛熊区分度 | 14.2 | 牛熊均值差 |
| 同期相关系数 | 0.845 | 热度 vs 上证综指 |
| 极热信号（≥80）后 60 日 | -7.9% | 177 次信号，71% 概率下跌 |
| 极冷信号（≤20）后 60 日 | +5.7% | 232 次信号，60% 概率上涨 |

> **结论**: V2 指标体系擅长"极端区域预警"和"同期热度跟踪"，但不擅长短期涨跌预测。极热区是可靠的减仓信号（历史胜率 71%），极冷区是可靠的建仓信号（历史胜率 60%）。
...
| **资金** | 15% | 两融余额市值比(15%) | 杠杆水位 |
| **情绪** | 42% | 涨停封板率(15%)、成交额 M2 比(15%)、换手率(12%) | 追涨情绪 + 市场活跃度 |
| **结构** | 15% | 创新高占比(10%)、MA 排列比(5%) | 内部结构健康度 |
...
| **S5** | **S5_calc** | **V2 引擎：8 指标 + 恐慌指数 + 维度加权合成** | **5–15s** |
...
| 文件 | 说明 |
|------|------|
| `index.json` | 最新热度（综合分 + 4 维度分 + 8 指标原始值） |
| `detail.json` | 含完整指标明细（8 指标百分位评分 + 原始值 + 恐慌指数成分分解） |
| `history.json` | 历史热度序列 |
| `indicator_history.json` | 8 指标历史趋势 |
...
- **明细**：8 个核心指标历史趋势图（含牛熊均线参考线和极值标记线）+ 恐慌指数卡片（含公式说明、三品种成分值、集中度）
- **记录**：近期每日热度得分明细表
...
│   ├── indicators/
│   │   ├── heat_index_v2.py         # ⭐ V2 引擎 — 每日流水线所用（4 维度 8 指标）
...
*版本: v3.22 | 调整: 2026-08-11 | 指标权重优化（基于 2814 交易日回测数据，涨停封板率 28%→15%，成交额M2比 10%→15%，创新高 6%→10%）；涨停封板率改为本地计算（stock_daily OHLC 自连接，不依赖 tushare API）；数据库 schema v10（新增 daily_seal_rate 表，历史回填 2812 条）；全历史回测验证牛熊预测能力*
