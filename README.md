...
| **东方财富curl** | 恒生AH股溢价指数 HSAHP (secid=100.HSAHP) | 间歇性封锁IP | — |
...
| # | 指标 | 数据源 | 标准化 | 选取意义 |
|---|------|--------|--------|---------|
| 5 | **融资买入占比** | tushare margin.rzmre / 全市场成交额 | 10年分位 | 杠杆资金情绪的直接度量；2015杠杆牛中融资余额比达历史峰值，是牛市核心驱动力 |
| 6 | **北向资金方向** | tushare moneyflow_hsgt | 近20日净买入比 | 外资对A股的配置意愿；持续净流入=看好=高分，持续净流出=看空=低分 |

### 情绪维度 (5项)

| # | 指标 | 数据源 | 标准化 | 选取意义 |
|---|------|--------|--------|---------|
| 7 | **换手率** | amount / circ_mv | 10年分位 | 市场活跃度的最直接指标；牛市顶峰换手率极高（2015年日换手率>3%） |
| 8 | **上涨/下跌家数比** | pctChg>0 占比 / pctChg<0 占比 | 10年分位 | 市场广度指标；牛市普涨时比值高，结构性牛市时比值低（2021年仅0.39） |
| 9 | **涨停占比** | pctChg ≥ 9.9% | 10年分位 | 极端乐观情绪；涨停潮=情绪过热=高分 |
| 10 | **跌停占比** | pctChg ≤ -9.9% | 10年分位(反向) | 极端恐慌情绪；跌停潮=情绪过冷=低分（反向计分） |
| 11 | **波动率** | 20日收益率标准差 | 10年分位 | VIX替代指标；高波动=分歧大/恐慌=高分（牛末熊初波动均大） |

### 技术维度 (4项)

| # | 指标 | 数据源 | 标准化 | 选取意义 |
|---|------|--------|--------|---------|
| 12 | **站上年线比例** | close > MA250 占比 | 静态分位 | 市场整体趋势强度；牛市末期>90%，熊市末期<10% |
| 13 | **创新高占比** | close = 250日最高 占比 | 静态分位 | 市场动量；牛市顶峰大量股票创新高，熊市几乎为0 |
| 14 | **均线偏离度** | close / MA250 - 1 | 10年分位 | 价格偏离长期均值的程度；过度偏离=泡沫=高分 |
| 15 | **量价背离** | 价格趋势 vs 量比 | 状态打分 | 价涨量缩=背离=高分(牛市末期常见)，价跌量缩=低分 |

### 结构维度 (2项)

| # | 指标 | 数据源 | 标准化 | 选取意义 |
|---|------|--------|--------|---------|
| 16 | **行业分化度** | 各行业pctChg 标准差 | 静态阈值(反向) | 低分化(普涨)=全面牛市=高分；高分化(结构性)=局部行情=低分 |
| 17 | **AH股溢价指数** | 恒生AH溢价指数 HSAHP | 历史分位 | HSAHP高=A股相对H股贵=高分；HSAHP低=A股便宜=低分。数据源: 东方财富 push2his API |
...
ah_premium              — AH溢价指数 OHLC (trade_date, open, close, high, low)
...
交易日 16:30 触发 (copaw cron / GitHub Actions)
  ...
  ├─ Step 4.5: 东方财富curl 拉取 HSAHP 日线 (全量更新, 含5次重试)
  │
...
| 2025-05-29 | 当前(3348) | 3348 | **61.5🟡** | 59.0 | 99.3 | 44.6 | 44.7 | 60.0 |
...
- [ ] 前端联调: web/data/index.json 渲染验证（仪表盘/雷达图/历史走势）
- [ ] GitHub Push: 创建仓库 + 推送代码 + 配置 GitHub Actions
- [ ] Phase 2 (6/24): 行业热度指数 + 板块热力图
- [ ] Phase 3: 单元测试 + 文档完善
