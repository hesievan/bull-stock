...
def fmt(v):
    ...
    if isinstance(v, tuple): return " / ".join(f"{x:.4f}" if isinstance(x,(int,float)) else str(x) for x in v)
    if isinstance(v, float): return f"{v:.4f}"
    return str(v)

for k in ['pe','buffett','margin_ratio_v2','north_ratio','yield_spread','m1_m2_spread','seal_rate','turnover_m2','turnover','new_high','ma_alignment']:
    eng = raw.get(k)
...
# margin_ratio_v2 复算
if m_mv and c_mv:
    print(f"margin_ratio_v2: 引擎={raw['margin_ratio_v2']:.4%}  复算={m_mv/c_mv:.4%}  两融余额={m_mv/1e8:.0f}亿 流通市值={c_mv/1e8:.0f}亿")
# north_ratio 复算
if nn is not None and amt:
    print(f"north_ratio: 引擎={raw['north_ratio']:.4%}  复算={nn/amt:.4%}  北向净={nn/1e8:.2f}亿 成交={amt/1e8:.0f}亿")
# yield_spread 复算
if y10 and y2:
...
if amt2 and m2b:
    amt_yuan = amt2*1000
    m2_yuan = m2b*1e8
    print(f"turnover_m2: 引擎={raw['turnover_m2']:.4%}  复算=amount/m2={amt_yuan/m2_yuan:.4%}  (成交={amt2/1e8:.0f}亿千元? m2={m2b}亿)")
# buffett 复算
gdp_all = conn.execute("SELECT quarter,gdp FROM gdp_quarterly WHERE gdp IS NOT NULL").fetchall()
yrs={}
for q,g_ in gdp_all: yrs[int(q[:4])]=yrs.get(int(q[:4]),0)+g_
cur_gdp = yrs.get(2025,0)*1e8
print(f"buffett: 引擎={raw['buffett']:.4%}  总市值={buf_mv*1e4/1e12:.2f}万亿 GDP(2025)={cur_gdp/1e12:.2f}万亿 比值={buf_mv*1e4/cur_gdp:.4%}")
...
print("="*72)
range_checks = [
    ('pe', "SELECT MIN(pe_med),MAX(pe_med),COUNT(*),SUM(pe_med IS NULL) FROM index_daily_pe WHERE trade_date>='2015-01-01'"),
    ...
    ('turnover', "SELECT MIN(turnover_rate),MAX(turnover_rate),COUNT(*),SUM(turnover_rate IS NULL) FROM daily_turnover"),
    ('new_high', "SELECT MIN(new_high_ratio),MAX(new_high_ratio),COUNT(*),SUM(new_high_ratio IS NULL) FROM daily_new_high"),
    ('ma_alignment', "SELECT MIN(ma_alignment_ratio),MAX(ma_alignment_ratio),COUNT(*),SUM(ma_alignment_ratio IS NULL) FROM daily_ma_alignment"),
...
for r in conn.execute("SELECT trade_date, north_net FROM northbound_history WHERE trade_date BETWEEN '2024-08-15' AND '2024-09-05' ORDER BY trade_date"):
