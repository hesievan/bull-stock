...
def main():
    ...
    idx_df = pd.read_sql("SELECT trade_date, close FROM index_daily WHERE index_code='sh000001' ORDER BY trade_date", conn)
    ...
    for i, td in enumerate(all_dates):
        ...
        if len(cur_pe_row) > 0:
            cur_pe = cur_pe_row.iloc[-1]["pe_med"]; cur_n = cur_pe_row.iloc[-1]["n_stocks"]
            ...
            if cur_n > 0 and len(hist_pe) > 60:
                ...
                if cur_n >= 600: lo = max(lo, 450)
                hist_pe = hist_pe[hist_pe["n_stocks"].between(lo, hi)]
            if len(hist_pe) >= 60:
                scores["pe"] = max(0, min(100, _pct_rank(hist_pe["pe_med"], cur_pe) * 100)); raws["pe"] = cur_pe
        ...
        if len(cur_buffett_row) > 0:
            ...
            if len(hist_buffett) >= 60:
                scores["buffett"] = max(0, min(100, _pct_rank(hist_buffett["buffett_ratio"], cur_br) * 100)); raws["buffett"] = cur_br
        ...
        if len(cur_margin_row) > 0:
            ...
            if len(hist_margin) >= 60:
                ...
                sc = pct * 100 if pct <= SATURATION_CUTOFF else (SATURATION_CUTOFF + SATURATION_HEADROOM * (1 - math.exp(-(pct - SATURATION_CUTOFF) * 20))) * 100
                scores["margin_ratio"] = max(0, min(100, sc)); raws["margin_ratio"] = cur_mr
        ...
        if len(cur_seal) > 0:
            ...
            if len(hist_seal) >= 60:
                scores["seal_rate"] = max(0, min(100, _pct_rank(hist_seal["seal_rate"], cur_sr) * 100)); raws["seal_rate"] = cur_sr
        ...
        if len(cur_tm2) > 0 and pd.notna(cur_tm2.iloc[0]["turnover_m2"]):
            ...
            if len(m2_merged) >= 60:
                scores["turnover_m2"] = max(0, min(100, _pct_rank(m2_merged["ratio"], cur_tm2_val) * 100)); raws["turnover_m2"] = cur_tm2_val
        ...
        if len(cur_turnover) > 0:
            ...
            if len(hist_tr) >= 60:
                scores["turnover"] = max(0, min(100, _pct_rank(hist_tr["turnover_rate"], cur_tr) * 100)); raws["turnover"] = cur_tr
        ...
        if len(cur_nh) > 0:
            ...
            if len(hist_nh) >= 60:
                scores["new_high"] = max(0, min(100, _pct_rank(hist_nh["new_high_ratio"], cur_nh_val) * 100)); raws["new_high"] = cur_nh_val

        cur_ma = ma_align_df[ma_align_df["trade_date"] == td]
        if len(cur_ma) == 0: cur_ma = ma_align_df[ma_align_df["trade_date"] <= td]
        if len(cur_ma) > 0:
            ...
            if len(hist_ma) >= 60:
                scores["ma_alignment"] = max(0, min(100, _pct_rank(hist_ma["ma_alignment_ratio"], cur_ma_val) * 100)); raws["ma_alignment"] = cur_ma_val
        ...
        for dim_name in DIMS:
            ...
            if not available:
                dim_scores[dim_name] = None; continue
        ...
        composite = sum(v * WEIGHTS[k] for k, v in valid_scores) / sum(WEIGHTS[k] for k, _ in valid_scores) if valid_scores else None
    ...
    def metrics(col):
        ...
        bull = d[d["is_bull"]]; bear = d[d["is_bear"]]
        bull_hit = (bull[col] >= 55).sum(); bear_hit = (bear[col] < 40).sum()
        corr = d[col].corr(d["close"])
        eh = d[d[col] >= 80]; el = d[d[col] <= 20]
        red = d[d[col] >= 65]
        def fwr(s):
            r = s["trade_date"].apply(lambda x: fwd_return(x, 60))
            r = r.dropna()
            return (round(r.mean(), 1), round((r > 0).mean() * 100), len(r)) if len(r) else (None, None, 0)
        eh_r = fwr(eh); el_r = fwr(el); red_r = fwr(red)
        return {
    ...
    for name in gated:
        ...
        if len(flipped) == 0:
            print(f"  {name}: 无摘红天数"); continue
        r = flipped["trade_date"].apply(lambda x: fwd_return(x, 60)).dropna()
        print(f"  {name}: 摘红 {len(flipped)} 天 | 其后60日 均值 {r.mean():.1f}% 胜率 {(r>0).mean()*100:.0f}% (n={len(r)})")
    ...
    print("=" * 96)
    key_dates = [
        ("2015-06-12", "5178大顶"), ("2015-08-26", "股灾底2850"),
        ("2016-01-28", "熔断底2638"), ("2018-01-29", "蓝筹牛顶3587"),
        ("2019-04-19", "春季顶3288"), ("2021-02-18", "核心资产顶3731"),
        ("2021-12-13", "结构牛顶"), ("2024-02-05", "底部2635"),
        ("2024-09-24", "924起点"), ("2024-10-08", "924顶3674"),
        ("2026-08-07", "最新(基准日)"), ("2026-08-11", "今日"),
    ]
    show_cols = ["composite_score", "g_UD<0.5|cap64", "g_UD<0.5|cap55"]
    print(f"{'日期':12s} | {'事件':14s} | {'BASE':>6s} | {'UD|64':>6s} | {'UD|55':>6s} | {'结构':>6s} | {'涨跌比':>7s} | {'上证':>7s}")
    print("-" * 88)
    for kd, desc in key_dates:
        row = df[df["trade_date"] == kd]
        if len(row) == 0: continue
        r = row.iloc[0]
        def fmt(v): return f"{v:.1f}" if pd.notna(v) else "—"
        udr = r["up_down_ratio"]
        print(f"  {kd} | {desc:14s} | {fmt(r['composite_score']):>6s} | {fmt(r['g_UD<0.5|cap64']):>6s} | "
    ...
    for name in gated:
        ...
        flip[name] = {"n_flipped": len(f), "fwd60_mean": round(r.mean(), 1) if len(r) else None,
