...
DIM_WEIGHTS = {  # 维度 → 指标 → 维度内权重（归一化后 0-1）
    ...
    "资金": {
        ...
        "m1_m2_spread": 3 / 15,
        "southbound": 1 / 15,
        "margin_buy_ratio": 3 / 15,
    },
    "情绪": {
        "seal_rate": 6 / 35,
        "turnover_m2": 14 / 35,
        "turnover": 9 / 35,
        "futures_discount": 2 / 35,
        "amplitude": 2 / 35,
        "realized_vol": 2 / 35,
    },
...
def main() -> None:
    ...
    lines.append("")
    lines.append(
        f"> 数据来源：`{CSV_PATH.name}`（{len(df)} 个交易日，"
    ...
    lines.append("|---|---|---|---|")
    lines.append(
        f"| 热度 vs 指数**水平**（同期） | {df['composite_score'].corr(df['close']):.3f} | {df['close'].notna().sum()} | "
    ...
    lines.append("")
    lines.append(
        f"同期水平相关 **{df['composite_score'].corr(df['close']):.3f}**（summary.json 0.816 口径）；"
    ...
    lines.append("")
    lines.append(
        "> 说明：方向一致率在 46%~51% 之间、接近随机水平，印证热度对**短期方向**预测能力弱，"
    ...
    for lb in labels:
        ...
        if lb in ("≤20", "20-30"):
            ...
            sig = "中性"
        lines.append(
            f"| {lb} | {len(sub)} | {fmt(m20 * 100)}% | {fmt(m60 * 100)}% | {fmt(med60 * 100)}% | {pct(up60, 0)} | {sig} |"
        )
    lines.append("")
    ...
    for thr, name in [(80, "极热 ≥80"), (70, "高热 ≥70"), (20, "极冷 ≤20"), (30, "低冷 ≤30")]:
        ...
        lines.append(
            f"| {name} | {len(sub)} | {pct(u5, 0)} | {pct(u20, 0)} | {pct(u60, 0)} | {fmt(m60 * 100)}% | {verdict} |"
        )
    lines.append("")
    ...
    lines.append("")
    lines.append(
        "三分类口径：**牛市** = bull_peak + bull_rally + slow_bull；**熊市** = bear_crash + bear_bottom；"
    ...
    for g in ["牛市", "熊市", "震荡"]:
        sub = df[df["group"] == g]
        lines.append(
            f"| {g} | {len(sub)} | {fmt(sub['composite_score'].mean())} | {fmt(sub['composite_score'].median())} | "
    ...
    lines.append("")
    lines.append(
        "> **解读**：牛市正确率最高（热度中枢高），熊市次之（熊市低热日占比近半），"
    ...
    for dstr, ev in KEY_EVENTS:
        ...
        hit = (
            "✅ 命中"
            if ((is_top and r["composite_score"] >= 65) or ((not is_top) and r["composite_score"] <= 30))
            else "⚠️ 漏报"
        )
        if ev == "924 行情起点":
            hit = "——"
        lines.append(
            f"| {dstr} | {ev} | **{fmt(r['composite_score'])}** | {r['level']} | "
            f"{fmt(f60 * 100, 1)}% | {fmt(dims['估值'], 1)} | {fmt(dims['资金'], 1)} | "
            f"{fmt(dims['情绪'], 1)} | {fmt(dims['结构'], 1)} | {hit} |"
        )
    lines.append("")
    lines.append(
        "> **误判一（顶部漏报）**：2018-01-29 蓝筹牛顶（热度 53.1）、2019-04-19 春季顶（热度 63.2）"
    ...
    lines.append("")
    lines.append(
        "> **误判二（底部漏报）**：2015-08-26 股灾底（33.2）、2020-03-23 疫情底（34.9）、2022-04-27 熊市底（34.4），"
    ...
    lines.append(
        f"极热信号（≥80）共 {len(hot)} 次，其中后 60 日仍上涨 >5% 的**假顶部** {len(false_top)} 次"
    ...
    for _, r in false_top.head(5).iterrows():
        lines.append(
            f"| {r['trade_date'].date()} | {fmt(r['composite_score'])} | **+{fmt(r['fwd60'] * 100, 1)}%** | {r['phase']} |"
        )
    lines.append("")
    lines.append(
        f"极冷信号（≤20）共 {len(cold)} 次，其中后 60 日仍下跌 >5% 的**假底部** {len(false_bot)} 次"
    ...
    lines.append("")
    lines.append(
        "> 2015-06-12 大顶前热度早已长期 ≥80（2015-03 起即在高位），极热信号**大幅提前**而非滞后，"
    ...
    lines.append("")
    lines.append(
        f"牛市热度均值 **{fmt(bull['composite_score'].mean())}** vs 熊市 **{fmt(bear['composite_score'].mean())}**，"
    ...
    lines.append("")
    lines.append(
        f"1. **本质是水平状态指标，不是短期择时器**：与指数水平同期相关 {s['corr_heat_vs_index']:.2f}，"
        "与未来 60 日收益相关 −0.2 左右。它回答的是“现在市场处于什么温度”，"
        "而非“明天涨跌”——**方向一致率接近随机**（46%~51%，A2）。"
    )
    lines.append(
        "2. **极端区间信号有效，中间区间无效**：极热（≥80）后 60 日上涨占比 27%、均值 −7.4%；"
        "极冷（≤20）后 60 日上涨占比 64%、均值 +7.9%；但 40-60 中性区间的未来收益无区分度（A3）。"
    )
    lines.append(
        "3. **牛熊识别可用，震荡市天然劣势**：牛市热度 ≥55 占 5 成以上、熊市 <40 占近半；"
        "但震荡市（反弹/回调）中热度频繁越界，假信号集中于此（B2）。"
    )
    lines.append(
        "4. **漏报集中在“单维驱动顶”与“资金底”**：2018 蓝筹顶（估值独热）、2019 春季顶（资金未确认）漏报；"
        "暴跌后资金维度（两融/北向/利差）偏热拖累底部信号（C1）。"
    )
    lines.append(
        "5. **个别指标方向性可疑**：futures_discount、amplitude 区分度为负或接近 0（B3），"
    ...
    lines.append("|---|---|---|---|")
    lines.append(
        "| 1 | **信号阈值化而非连续分位**：只把 ≥80（离场）/≤20（观察）当信号，中段一律视为无信号，"
        "减少震荡市假信号 | A3/A4：极端区间命中率远高于中间区间 | 小 |"
    )
    lines.append(
        "| 2 | **增加顶部确认机制**：热度从 ≥70 回落 10 分且结构维破位（new_high<50 或 ma_alignment 回落）"
        "触发离场，可覆盖 2018-01/2019-04 类“单维驱动顶”漏报与 2024-10 类“顶不热” | C1 误判一 | 中 |"
    )
    lines.append(
        "| 3 | **底部信号改“温度计”用法**：≤20 仅提示“低估值区+情绪冰点”，不单独作为进场依据，"
        "与本项目定位（仅离场/减仓）一致 | C1 误判二：底部资金维度偏高 | 小 |"
    )
    lines.append(
        "| 4 | **压缩或剔除负区分度指标**：futures_discount（−3.9）、amplitude（−8.3）降权或转为展示项；"
        "或对它们做方向翻转后再入分 | B3 区分度表 | 小 |"
    )
    lines.append(
        "| 5 | **按 regime 自适应阈值**：用滚动 1260 日分位的相对位置（如 P90/P10）替代绝对 80/20 阈值，"
        "缓解 2015 型大牛与 2024 型快牛阈值敏感度差异 | C4 中枢漂移 | 中 |"
    )
    lines.append(
        "| 6 | **震荡市独立模型**：bounce/correction 阶段用“热度分位数变化率”（Δscore）而非绝对水平"
        "做区间交易参考；或明确放弃震荡市择时 | B2 震荡正确率最低 | 大 |"
    )
    lines.append(
        "| 7 | **补充验证集**：以上结论基于 2015-2026 单一市场样本，建议按 2015-2020 / 2021-2026 两段"
