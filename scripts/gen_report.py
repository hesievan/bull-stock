...
for _k, _label, _fmt, _note in [
    ("pe", "大盘PE", "%.1f", "分"),
    ("buffett", "巴菲特指标", "%.1f", "分"),
    ("margin_ratio_v2", "两融余额市值比", "%.1f", "分"),
    ("seal_rate", "涨停封板率", "%.1f", "分"),
    ("turnover_m2", "成交额M2比", "%.1f", "分"),
    ("turnover", "换手率", "%.1f", "分"),
    ("new_high", "创新高占比", "%.1f", "分"),
    ...
    if v is not None:
        _hl.append((_label, _fmt % v, _note))
# 展示指标 (原始值, 不参与V2计算)
...
if qv is not None:
    _hl.append(("QVIX恐慌指数", "%.1f" % qv, "展示不计分"))
...
for h in hist[-10:]:
    md.append(
        "| %s | %.1f | %s %s |" % (h["trade_date"], h["composite_score"], LEVEL_EMOJI.get(h["level"], "⚪"), h["level"])
    )
md.append("")
md.append("---")
md.append("")
...
with open(os.path.join(OUTPUT_DIR, "daily_%s.md" % td_clean), "w", encoding="utf-8") as f:
    f.write("\n".join(md))
...
for k in DIM_ORDER:
    s = dim_scores.get(k)
    dc = score_color(s)
    pct = min(max(s, 0), 100) if s else 0
    ps = prev_dims.get(k)
    sl = score_label(s)
    d_html = dim_delta_str(s, ps) if ps else ""
    if s is None:
        dim_cards += (
            '<div class="dim-card na"><div class="dim-header"><span class="dim-name">%s</span><span class="dim-weight">%s</span></div><div class="dim-score">—</div><div class="dim-eval">数据暂缺</div></div>'
            % (DIM_LABELS[k], DIM_WEIGHTS[k])
        )
    else:
        dim_cards += (
            '<div class="dim-card"><div class="dim-header"><span class="dim-name">%s</span><span class="dim-weight">%s</span></div><div class="dim-score" style="color:%s">%.0f<span class="dim-delta">%s</span></div><div class="dim-eval" style="color:%s">%s</div><div class="dim-bar"><div class="dim-bar-fill" style="width:%d%%;background:%s"></div></div></div>'
            % (DIM_LABELS[k], DIM_WEIGHTS[k], dc, s, d_html, dc, sl, int(pct), dc)
        )

hl_html = "".join(
    [
        '<li><span class="hl-name">%s</span><span class="hl-val">%s</span><span class="hl-note">%s</span></li>\n'
        % (n, v, t)
        for n, v, t in _hl
    ]
)
...
if prev_score is not None:
    ...
    delta_html = '<div class="delta">较上一交易日 <span style="color:%s">%s %.1f (%s)</span></div>' % (
        dc,
        arrow,
        abs(d),
        trend_label,
...
status_items = "".join(
    [
        '<div class="status-item">%s <code>%s</code> <span class="%s">%s</span> <span class="elapsed">%.1fs</span></div>'
        % (
            "✅" if v["status"] == "OK" else ("⏭️" if v["status"] == "SKIPPED" else "❌"),
            sn,
            v["status"].lower(),
            v["status"],
            v.get("elapsed", 0),
        )
        for sn, v in status["steps"].items()
...
with open(os.path.join(OUTPUT_DIR, "daily_%s.html" % td_clean), "w", encoding="utf-8") as f:
    f.write(out)
...
try:
    ...
    def _f(sz):
        try:
            return IF.truetype(_font_path, sz)
        except Exception:
    ...
    def _bar(cx, cy, w, sc, color, h=6):
        ...
        if fw > 0:
            draw.rectangle([(cx, cy), (cx + fw, cy + h)], fill=color)
    ...
    t = "A股牛市热度指数日报"
    _txt_center(t, y + 12, C["bright"], 20, True)
    m = "📅 %s  ·  🕐 %s  ·  📈 日频  ·  v3.5" % (trade_date, now_str)
    _txt_center(m, y + 42, C["dim"], 11)
    y += 88

    # 综合得分
    _rr([(PAD, y), (W - PAD, y + 200)], r=10, fill=C["card"], outline=C["border"])
    ss = "%.1f" % score
    _txt_center(ss, y + 16, _lc, 64, True)
    _txt_center(level_cn, y + 90, _lc, 16, True)
    if level_label:
        _txt_center(level_label, y + 114, C["dim"], 11)
    bar_y = y + 134
    _bar(PAD + 80, bar_y, W - 2 * PAD - 160, score, _lc, 8)
    for tick, tx in [(0, "0"), (40, "40"), (65, "65"), (100, "100")]:
        tx_x = PAD + 80 + int((W - 2 * PAD - 160) * tick / 100) - 8
        draw.text((tx_x, bar_y + 12), tx, fill=C["muted"], font=_f(9))
    if prev_score is not None:
        d = score - prev_score
        arrow = "▲" if d > 0 else ("▼" if d < 0 else "─")
        dc = C["red"] if d > 0 else C["green"]
        ds = "较上一交易日: %s %.1f" % (arrow, abs(d))
        _txt_center(ds, y + 156, dc, 11)
    y += 216

    # 六维度拆解
    _txt("📊 六维度拆解", PAD, y, C["bright"], 16, True)
    y += 8
    cw = (W - 2 * PAD - 5 * 8) // 6
    ch = 96
    cy0 = y
    for i, dk in enumerate(DIM_ORDER):
        ...
        scol = score_color(s)
        cx = PAD + i * (cw + 8)
        _rr([(cx, cy0), (cx + cw, cy0 + ch)], r=8, fill=C["card"], outline=C["border"])
        draw.text((cx + 8, cy0 + 8), DIM_LABELS[dk], fill=C["dim"], font=_f(11))
        bbox = _f(9).getbbox(DIM_WEIGHTS[dk])
        draw.text((cx + cw - (bbox[2] - bbox[0]) - 8, cy0 + 10), DIM_WEIGHTS[dk], fill=C["muted"], font=_f(9))
        ss2 = "%.0f" % sc if s else "—"
        bbox = _f(32).getbbox(ss2)
        draw.text((cx + (cw - (bbox[2] - bbox[0])) // 2, cy0 + 28), ss2, fill=scol, font=_f(32))
        sl = score_label(s)
        bbox = _f(10).getbbox(sl)
        draw.text((cx + (cw - (bbox[2] - bbox[0])) // 2, cy0 + 66), sl, fill=scol, font=_f(10))
        _bar(cx + 8, cy0 + 82, cw - 16, sc, scol, 4)
    y = cy0 + ch + 24

    # 关键指标
    _txt("📌 关键指标", PAD, y, C["bright"], 16, True)
    y += 8
    col_w = (W - 2 * PAD - 12) // 2
    ...
    # 历史参考
    _txt("📊 历史参考", PAD, y, C["bright"], 14, True)
    y += 8
    refs = [
        ("2015-06-12", "牛市顶 (上证5178)", "73.8", C["red"]),
        ("2021-02-18", "牛市顶 (上证3731)", "66.2", C["dim"]),
        ("2024-10-08", "脉冲顶 (上证3489)", "65.1", C["dim"]),
    ...
    for dt2, st2, sc2, sc2c in refs:
        ...
        bbox = _f(12).getbbox(sc2)
        draw.text((W - PAD - 12 - (bbox[2] - bbox[0]), y + 6), sc2, fill=sc2c, font=_f(12))
        y += 32
    y += 16

    # 运行状态
    draw.line([(PAD, y + 10), (W - PAD, y + 10)], fill=C["border"], width=1)
    y += 20
    ft = "⚙️ 运行状态: %d OK / %d FAILED / %d SKIPPED" % (n_ok, n_fail, n_skip)
    _txt_center(ft, y, C["muted"], 10)
    y += 20
    wt = "⚠️ 不构成投资建议，仅供参考  ·  v3.5"
    _txt_center(wt, y, C["muted"], 10)
    y += 24
