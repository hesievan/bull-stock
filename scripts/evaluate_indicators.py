...
def spearman(a, b):
    ...
    ra = pd.Series(a[m]).rank(); rb = pd.Series(b[m]).rank()
...
def tstat_bullbear(v, bull, bear):
    ...
    b = v[bull]; r = v[bear]
    b = b[np.isfinite(b)]; r = r[np.isfinite(r)]
    if len(b) < 30 or len(r) < 30: return np.nan
...
for k in KEYS:
    x = df[COL[k]]
    ic20_all = spearman(x, df["ret20"]); ic60_all = spearman(x, df["ret60"])
    # walk-forward 分段
    seg_ic = []
    for s in range(3):
        m = df["seg"] == s
    ...
    rowsA.append(dict(k=k, ic20=ic20_all, ic60=ic60_all, t60=t,
                      seg0=seg_ic[0], seg1=seg_ic[1], seg2=seg_ic[2],
                      stable=same_sign, mono=mono, n=n))
...
for k in KEYS:
    ...
    rowsC.append(dict(k=k, t=t,
                      bull=v[bull_mask].mean(), bear=v[~bull_mask].mean()))
...
for k in KEYS:
    ...
    c = composite(sub)
    ic60 = spearman(c, df["ret60"]); ic20 = spearman(c, df["ret20"])
...
for i in range(1, len(state)):
    if state[i-1] == 1 and state[i] == 0: down_ev.append(i)
    if state[i-1] == 0 and state[i] == 1: up_ev.append(i)
...
for k in KEYS:
    ...
    for nm, evs in [("down", down_ev), ("up", up_ev)]:
        ...
        for i in evs:
            ...
            if len(seg) >= 10: wins.append(seg.mean())
        if wins:
            z = (np.mean(wins) - hist_mean) / hist_std if hist_std > 0 else 0
            out[nm+"_mean"] = np.mean(wins); out[nm+"_z"] = z
        else:
            out[nm+"_mean"] = np.nan; out[nm+"_z"] = np.nan
    rowsE.append(out)
...
out = {
  ...
  "meta": {"n": len(df), "range": [df.trade_date.iloc[0], df.trade_date.iloc[-1]],
           "down_events": [dates[i] for i in down_ev], "up_events": [dates[i] for i in up_ev]},
}
